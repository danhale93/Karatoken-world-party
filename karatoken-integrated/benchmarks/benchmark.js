...
async function runBenchmark() {
  ...
  for (const test of testCases) {
    ...
    const currentResult = await currentGenreSwap(test.genre);
    ...
    const stylusResult = await stylusTransfer(test.genre);
...
runBenchmark()
  ...
  .then(summary => {
    ...
    const result = {
      timestamp,
      summary,
      details: testCases.map((test, i) => ({
        ...test,
        currentTime: results[i].currentTime,
        stylusTime: results[i].stylusTime,
        difference: results[i].difference
      }))
    };
    
    fs.writeFileSync(`benchmark-results-${timestamp}.json`, JSON.stringify(result, null, 2));
