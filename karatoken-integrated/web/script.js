(() => {
  ...
  copyLrcBtn?.addEventListener("click", async () => {
    ...
    if (
      !text ||
      text === "LRC will appear here…" ||
      text === "Failed to load LRC"
    )
      return;
    try {
  ...
  function createVideoElement(video) {
    ...
    div.setAttribute(
      "aria-label",
      `Select video: ${safeTitle} by ${safeChannel}`,
    );
    div.innerHTML = `
  ...
  async function ytSearch() {
    ...
    try {
      const response = await fetch(
        `${API_BASE}/api/youtube/search?q=${encodeURIComponent(q)}`,
      );
      ...
      if (!data.items || data.items.length === 0) {
        ...
        document
          ...
          ?.addEventListener("click", ytSearch);
        return;
      }
  ...
  async function ytDownload() {
    ...
    if (!url) {
      ytDlOut.innerHTML =
        '<div class="error">Please enter or select a YouTube URL</div>';
      return;
    }
    ...
    if (!url.match(/^(https?:\/\/)?(www\.)?(youtube\.com|youtu\.?be)\/.+$/)) {
      ytDlOut.innerHTML =
        '<div class="error">Please enter a valid YouTube URL</div>';
      return;
    }
    ...
    try {
      ...
      document
        ...
        ?.addEventListener("click", ytDownload);
    } finally {
    ...
    try {
      ...
      while (true) {
        try {
          ...
          const job = js.job || {};
          const progress =
            typeof job.progress === "number" ? job.progress : lastProgress;
          ...
          if (job.status === "completed") {
            ...
            const outRel = job.outputUrl
              ? job.outputUrl.startsWith("http")
                ? new URL(job.outputUrl).pathname
                : job.outputUrl
              : null;
            const lrcRel = job.lrcUrl
              ? job.lrcUrl.startsWith("http")
                ? new URL(job.lrcUrl).pathname
                : job.lrcUrl
              : null;
            // Prefer relative URLs so they work through the preview proxy
            let outUrl =
              outRel && outRel.startsWith("/")
                ? outRel
                : outRel
                  ? `/${outRel}`
                  : null;
            let lrcUrl =
              lrcRel && lrcRel.startsWith("/")
                ? lrcRel
                : lrcRel
                  ? `/${lrcRel}`
                  : null;
            ...
            if (outUrl) {
              try {
                const h = await fetch(outUrl, { method: "HEAD" });
                if (!h.ok) throw new Error("bad");
              } catch (_) {}
            }
            if (lrcUrl) {
              try {
                const h = await fetch(lrcUrl, { method: "HEAD" });
                if (!h.ok) throw new Error("bad");
              } catch (_) {}
            }
            const outName = outUrl
              ? outUrl.split("/").pop() || "output.wav"
              : null;
            const lrcName = lrcUrl
              ? lrcUrl.split("/").pop() || "lyrics.lrc"
              : null;
            ...
            if (genreAudioEl && outUrl) {
              ...
              genreAudioEl.load().catch((e) => {
                ...
                msg.textContent =
                  "Audio preview failed to load. Try downloading the file instead.";
  ...
  async function swapGenre() {
    ...
    try {
      ...
      if (json.statusUrl) {
        ...
        genreSwapOut.textContent =
          "Started processing, but no status URL provided";
        if (genreSwapBtn) {
          genreSwapBtn.disabled = false;
          genreSwapBtn.classList.remove("button-loading");
          genreSwapBtn.removeAttribute("aria-busy");
  ...
  function updateSwapButtonState() {
    ...
    if (genreSwapBtn.disabled) {
      if (!hasAudio && !hasGenre) {
        genreSwapBtn.setAttribute(
          "title",
          "Please provide an Audio URL and Target Genre to process the swap",
        );
        genreSwapBtn.setAttribute(
          "aria-label",
          "Please provide an Audio URL and Target Genre to process the swap",
  ...
  function updateYtDownloadButtonState() {
    ...
    ytDlBtn.disabled = !isValid;
    if (ytDlBtn.disabled) {
      ytDlBtn.setAttribute("title", "Please enter or select a valid YouTube URL");
      ytDlBtn.setAttribute("aria-label", "Please enter or select a valid YouTube URL");
    } else {
      ytDlBtn.setAttribute("title", "Download Audio");
      ytDlBtn.setAttribute("aria-label", "Download Audio");
    }
  }
