...
## 2026-08-05 - Direct Character Code Parsing of Fixed-Width Timestamps in Subtitle Parser
**Learning:** Standard fixed-width subtitle timestamps (such as HH:MM:SS,mmm in SRT) can be parsed significantly faster by extracting integer values directly using character codes via `.charCodeAt(index) - 48`. This eliminates the massive overhead of regular expression compilation and execution, `parseFloat` casting, string replacements, and intermediate substring allocations, achieving a ~2.7x speedup on hot parsing loops.
**Action:** When parsing well-defined, fixed-width text patterns in hot pathways, access characters directly using `charCodeAt()` and use simple integer multiplication rather than regex matching or string slicing to maximize parsing throughput and minimize garbage collection.
