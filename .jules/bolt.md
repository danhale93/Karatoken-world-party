...
## 2026-08-05 - In-Place Mixing of Dry and Wet Signals in Local TypedArrays to Reduce Memory Allocation
**Learning:** In JS/TS audio effect functions (like reverb), allocating multiple duplicate-sized `Float32Array` buffers (one for wet signal computation, one for the mixed output result) increases memory allocation overhead and triggers frequent garbage collection cycles. Since the wet array is freshly allocated locally inside the effect function, we can safely overwrite it in-place during the dry-wet mixing loop and return it, reducing the function's memory allocations by 50% without mutating caller arguments or creating external side-effects.
**Action:** When designing JS/TS signal processors that generate a local temporary buffer, reuse that buffer in-place to perform final mixing/processing and return it, eliminating the need to allocate a separate result array.
